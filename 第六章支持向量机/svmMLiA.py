    ...
    while(iter < maxIter):
        ...
        for i in range(m):
            ...
            if ((labelMat[i]*Ei  < -toler) and (alphas[i] < C)) or ((labelMat[i]*Ei > toler) and (alphas[i]  > 0)):
                ...
                b2 = b - Ej - labelMat[i]*(alphas[j] - alphaJold)*dataMatrix[i,:]*dataMatrix[j,:].T-labelMat[j]*(alphas[j]-alphaJold)*dataMatrix[j,:]*dataMatrix[j,:].T
...
class optStruct:
    def __init__(self,dataMatIn,classLabels,C,toler,kTup):
        ...
        self.m = shape(dataMatIn)[0]
        import numpy
        self.alphas = mat(numpy.zeros((self.m,1)))
        self.b = 0
        self.eCache = mat(numpy.zeros((self.m,2)))
        self.K = mat(numpy.zeros((self.m,self.m)))
        for i in range(self.m):
            self.X[:,i] = kernelTrans(self.X,self.X[i,:],kTup)
...
def testRbf(k1 = 1.3):
    ...
    for i in range(m):
        ...
        # sign为符号函数，sign(x) x>0 sign(x) = 1 x < 0 sign(x) = -1 x = 0 sign(x) = 0
        if sign(predict) != sign(labelArr[i]):
            errorCount += 1
    print("the tarining error rate is: %f" %(float(errorCount)/m))
...
def loadImages(dirName):
    ...
    for i in range(m):
        ...
        filteStr = fileNameStr.split('.')[0]
        classNumStr = int(filteStr.split('_')[0])
        if classNumStr == 9:
            hwLabels.append(-1)
        else:
            hwLabels.append(1)
